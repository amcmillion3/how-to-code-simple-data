...
;; produce true if rocket's descent has ended; number of kilometers left to Earth if still descending. 
(check-expect (has-landed? 100) "100km above Earth's surface")
(check-expect (has-landed? 23) "23km above Earth's surface")
(check-expect (has-landed? 0.25) "1/4km above Earth's surface")
(check-expect (has-landed? false) true)

;(define (has-landed? rd) "")               stub

;(define (has-landed? rd)         template
;  (cond [(number? rd) (... rd)]
;        [(false? rd)  (...)]))

(define (has-landed? rd)
  (cond [(number? rd) (string-append (number->string rd) "km above Earth's surface")]
        [(false? rd)  true]))